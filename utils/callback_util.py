...
@register_callback("history")
class HistoryCallback(DiffusionCallback):
    def __init__(self, frequency: int, workdir: Path):
        super().__init__(frequency, workdir)
        self.history = []
    ...
    def save_json(self):
        ...
        with open(os.path.join(self.workdir, "history.json"), "w") as f:
            json.dump(self.history, f)

    @torch.no_grad()
    def callback(self, step, t, callback_kwargs):
        cfg_part = callback_kwargs.get("cfg_part")
        cfg_part_with_guid = callback_kwargs.get("guid")
        
        item = {}
        item["step"] = step
        item["t"] = t.item()
        item["cfg_part"] = torch.norm(cfg_part, p=2, dim=(1, 2, 3)).reshape(-1, 1, 1, 1).item()
        item["guid"] = torch.norm(cfg_part_with_guid, p=2, dim=(1, 2, 3)).reshape(-1, 1, 1, 1).item()
