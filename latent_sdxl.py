...
class SDXL():
    def __init__(self, 
        ...
        self.N_ts = len(self.scheduler.timesteps)
        self.scheduler.set_timesteps(solver_config.num_sampling, device=device)
        self.skip = self.N_ts // solver_config.num_sampling
        self.a = solver_config.a
        self.b = solver_config.b
        self.gamma = solver_config.gamma
    ...
    def get_from_beta(self, t, a, b, normalize=False):
        if normalize:
            assert a > 1.0 and b > 1.0
            t_max = (a - 1.0) / (a + b - 2.0)
            return beta.pdf(t, a, b) / beta.pdf(t_max, a, b)
        return beta.pdf(t, a, b)
    ...
    def reverse_process(self,
        ...
        for step, t in enumerate(pbar):
            ...
            with torch.no_grad():
                ...
                cfg_part = (noise_c - noise_uc)
                cfg_part_with_guid = cfg_guidance * cfg_part
                
                noise_pred = noise_uc + cfg_part_with_guid
...
@register_solver('ddim_geo')
class BaseDDIMGeo(SDXL):
    def reverse_process(self,
                        null_prompt_embeds,
                        prompt_embeds,
                        cfg_guidance,
        ...
        for step, t in enumerate(pbar):
            ...
            with torch.no_grad():
                ...
                # GEO implementation
                d = (noise_uc.shape[-1] * noise_uc.shape[-2] * noise_uc.shape[-3]) ** 0.5
                diff = noise_c - noise_uc
                diff_norm = torch.norm(diff, p=2, dim=(1, 2, 3)).reshape(-1, 1, 1, 1)
                factor = d/self.N_ts
                cfg_part = factor * (diff/diff_norm)
                cfg_part_with_guid = cfg_guidance * cfg_part
                noise_pred = noise_uc + cfg_part_with_guid
...
@register_solver('ddim_geo_beta')
class BetaDDIMGeo(SDXL):
    def reverse_process(self,
                        null_prompt_embeds,
                        prompt_embeds,
                        cfg_guidance,
        ...
        for step, t in enumerate(pbar):
            ...
            with torch.no_grad():
                noise_uc, noise_c = self.predict_noise(zt, t, null_prompt_embeds, prompt_embeds, add_cond_kwargs)       
                # GEO implementation with beta
                diff = noise_c - noise_uc
                diff_norm = torch.norm(diff, p=2, dim=(1, 2, 3)).reshape(-1, 1, 1, 1)
                cfg_part = (diff/((diff_norm)**self.gamma)) * self.get_from_beta(step/total_steps, self.a, self.b)
                cfg_part_with_guid = cfg_guidance * cfg_part
                noise_pred = noise_uc + cfg_part_with_guid
    ...
    def reverse_process(self,
        ...
        for step, t in enumerate(pbar):
            ...
            with torch.no_grad():
                noise_uc, noise_c = self.predict_noise(zt, t, null_prompt_embeds, prompt_embeds, add_cond_kwargs)
                diff = noise_c - noise_uc
                cfg_part = diff * self.get_from_beta(step/total_steps, self.a, self.b)
                cfg_part_with_guid = cfg_guidance * cfg_part
                noise_pred = noise_uc + cfg_part_with_guid
...
@register_solver("ddim_cfg++_beta")
class BaseDDIMCFGppBeta(SDXL):
    def reverse_process(self,
                        null_prompt_embeds,
                        prompt_embeds,
                        cfg_guidance,
        ...
        for step, t in enumerate(pbar):
            ...
            with torch.no_grad():
                noise_uc, noise_c = self.predict_noise(zt, t, null_prompt_embeds, prompt_embeds, add_cond_kwargs)
                cfg_part = (noise_c - noise_uc) * self.get_from_beta(step/total_steps, self.a, self.b)
                cfg_part_with_guid = cfg_guidance * cfg_part
                noise_pred = noise_uc + cfg_part_with_guid
